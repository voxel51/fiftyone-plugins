...
class EmbeddingsPanel(foo.Panel):
    ...
    def render(self, ctx):
        ...
        actions = panel.menu('menu', variant="square", width=100, align_y="center")
        ...
        if brain_key:
            ...
            if ctx.panel.state.selected:
                actions.btn('clear_selection', icon="clear", description="Clear Selection", on_click=self.on_deselect)
            actions.btn('clear_zoom', icon="center_focus_weak", description="Clear Zoom", on_click=self.on_click_clear_zoom)
            actions.btn('lasso_mode', icon="highlight_alt", description="Select", on_click=self.on_click_lasso_mode)
            actions.btn('pan_mode', icon="open_with", description="Pan", on_click=self.on_click_pan_mode)
            actions.btn('learn_more', icon="help", description="Learn More", on_click=self.on_click_learn_more)
