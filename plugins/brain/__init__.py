    ...
    if num_dims == 2 and Version(foc.VERSION) >= Version("1.4.0"):
        ...
        if create_index:
            ...
            kwargs.str(
                ...
                label="Points field",
                description=(
                    "The field name in which to store the spatial index"
                ),
            )
    ...
    if info.config.points_field is not None:
        warning = types.Warning(
            label=(
                ...
                f"`{info.config.points_field}` field. Would you like to "
                "remove it?"
            )
        )
        ...
        inputs.str(
            ...
            default=brain_key,
            label="Points field",
            description="The field name in which to store the spatial index",
        )

        # Database indexes are not currently supported for patch results
        if info.config.patches_field is None:
            inputs.bool(
                "create_index",
                default=True,
